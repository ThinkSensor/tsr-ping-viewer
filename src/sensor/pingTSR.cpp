...
PING_LOGGING_CATEGORY(PING_PROTOCOL_PINGTSR, "ping.protocol.pingtsr")

const int PingTSR::_pingMaxFrequency = 50;
...
{
    setName("Ping1DTSR");
    setControlPanel({"qrc:/Ping1DTSRControlPanel.qml"});
    ...
    connect(&_periodicRequestTimer, &QTimer::timeout, this, [this] {
        ...
        if (!link()->isOpen()) {
            qCCritical(PING_PROTOCOL_PINGTSR) << "Can't write, port is not open!";
            _periodicRequestTimer.stop();
            return;
        }
    ...
    connect(this, &Sensor::connectionOpen, this, &PingTSR::startPreConfigurationProcess);

    // Wait for device id to load the correct settings
    connect(this, &PingTSR::srcIdChanged, this, &PingTSR::setLastPingConfiguration);

    connect(this, &PingTSR::firmwareVersionMinorChanged, this, [this] {
        // Wait for firmware information to be available before looking for new versions
        static bool once = false;
        if (!once) {
            once = true;
            NetworkTool::self()->checkNewFirmware(
                "ping1dTSR", std::bind(&PingTSR::checkNewFirmwareInGitHubPayload, this, std::placeholders::_1));
        }
    });
}

void PingTSR::startPreConfigurationProcess()
{
    qCDebug(PING_PROTOCOL_PINGTSR) << "Start pre configuration task and requests.";
    if (!link()->isWritable()) {
        qCDebug(PING_PROTOCOL_PINGTSR) << "It's only possible to set last configuration when link is writable.";
        return;
    }
...
void PingTSR::loadLastPingConfigurationSettings()
{
    // Set default values
    ...
    if (pingConfigurationVariant.type() != QVariant::Map) {
        qCWarning(PING_PROTOCOL_PINGTSR) << "No valid PingConfiguration in settings." << pingConfigurationVariant.type();
        return;
    }
...
void PingTSR::updatePingConfigurationSettings()
{
...
void PingTSR::connectLink(LinkType connType, const QStringList& connString)
{
...
#pragma omp for
    ...
    default:
        qWarning(PING_PROTOCOL_PINGTSR) << "UNHANDLED MESSAGE ID:" << msg.message_id();
        break;
    }
...
void PingTSR::firmwareUpdate(QString fileUrl, bool sendPingGotoBootloader, int baud, bool verify)
{
...
void PingTSR::flash(const QString& fileUrl, bool sendPingGotoBootloader, int baud, bool verify)
{
    ...
    if (!HexValidator::isValidFile(fileUrl)) {
        ...
        qCWarning(PING_PROTOCOL_PINGTSR) << errorMsg;
    ...
    if (sendPingGotoBootloader) {
        qCDebug(PING_PROTOCOL_PINGTSR) << "Put it in bootloader mode.";
    ...
    while (serialLink->port()->bytesToWrite()) {
        qCDebug(PING_PROTOCOL_PINGTSR) << "Waiting for bytes to be written...";
        ...
        qCDebug(PING_PROTOCOL_PINGTSR) << "Done !";
    }

    qCDebug(PING_PROTOCOL_PINGTSR) << "Finish connection.";
    ...
    auto finishConnection = [=] {
        ...
        qCDebug(PING_PROTOCOL_PINGTSR) << "Save sensor configuration.";
        updatePingConfigurationSettings();

        qCDebug(PING_PROTOCOL_PINGTSR) << "Start flash.";
...
void PingTSR::setLastPingConfiguration()
{
    ...
    qCDebug(PING_PROTOCOL_PINGTSR).noquote() << output;
    ...
    connect(lastPingConfigurationTimer, &QTimer::timeout, this, [this] {
        ...
        for (const auto& key : _pingConfiguration.keys()) {
            ...
            if (key.contains("automaticMode") && dataStruct.value) {
                qCDebug(PING_PROTOCOL_PINGTSR) << "Device was running with last configuration in auto mode.";
        ...
        if (stopLastPingConfigurationTimer) {
            qCDebug(PING_PROTOCOL_PINGTSR) << "Last configuration done, timer will stop now.";
...
void PingTSR::setPingFrequency(float pingFrequency)
{
    if (pingFrequency <= 0 || pingFrequency > _pingMaxFrequency) {
        qCWarning(PING_PROTOCOL_PINGTSR) << "Invalid frequency:" << pingFrequency;
        do_continuous_stop(ContinuousId::PROFILE);
    } else {
        int periodMilliseconds = 1000.0f / pingFrequency;
        qCDebug(PING_PROTOCOL_PINGTSR) << "Setting frequency(Hz) and period(ms):" << pingFrequency << periodMilliseconds;
        set_ping_interval(periodMilliseconds);
        do_continuous_start(ContinuousId::PROFILE);
    }
    qCDebug(PING_PROTOCOL_PINGTSR) << "Ping frequency" << pingFrequency;
}

void PingTSR::resetSettings()
{
    qCDebug(PING_PROTOCOL_PINGTSR) << "Settings will be reseted.";
...
void PingTSR::printSensorInformation() const
{
    qCDebug(PING_PROTOCOL_PINGTSR) << "Ping1DTSR Status:";
    qCDebug(PING_PROTOCOL_PINGTSR) << "\t- board_voltage:" << _board_voltage;
    qCDebug(PING_PROTOCOL_PINGTSR) << "\t- pcb_temperature:" << _pcb_temperature;
    qCDebug(PING_PROTOCOL_PINGTSR) << "\t- processor_temperature:" << _processor_temperature;
    qCDebug(PING_PROTOCOL_PINGTSR) << "\t- ping_enable:" << _ping_enable;
    qCDebug(PING_PROTOCOL_PINGTSR) << "\t- distance:" << _distance;
    qCDebug(PING_PROTOCOL_PINGTSR) << "\t- confidence:" << _confidence;
    qCDebug(PING_PROTOCOL_PINGTSR) << "\t- transmit_duration:" << _transmit_duration;
    qCDebug(PING_PROTOCOL_PINGTSR) << "\t- ping_number:" << _ping_number;
    qCDebug(PING_PROTOCOL_PINGTSR) << "\t- start_mm:" << _scan_start;
    qCDebug(PING_PROTOCOL_PINGTSR) << "\t- length_mm:" << _scan_length;
    qCDebug(PING_PROTOCOL_PINGTSR) << "\t- gain_setting:" << _gain_setting;
    qCDebug(PING_PROTOCOL_PINGTSR) << "\t- mode_auto:" << _mode_auto;
    qCDebug(PING_PROTOCOL_PINGTSR) << "\t- ping_interval:" << _ping_interval;
    qCDebug(PING_PROTOCOL_PINGTSR) << "\t- points:" << QByteArray((const char*)_points.data(), _num_points).toHex(',');
}

void PingTSR::checkNewFirmwareInGitHubPayload(const QJsonDocument& jsonDocument)
{
    float lastVersionAvailable = 0.0;

    auto filesPayload = jsonDocument.array();
    for (const QJsonValue& filePayload : filesPayload) {
        qCDebug(PING_PROTOCOL_PINGTSR) << filePayload["name"].toString();
...
void PingTSR::resetSensorLocalVariables()
{
...
PingTSR::~PingTSR() { updatePingConfigurationSettings(); }

QDebug operator<<(QDebug d, const PingTSR::messageStatus& other)
{
